...
Actual standalone inference with the cached `mlx-community/gemma-3-1b-it-4bit` model returned malformed JSON and incorrect indexes on eight original synthetic news reports (4.66 seconds). A shorter experimental prompt also failed. The recorded response is checked into `shared/fixtures/semantic-model-failure.json` and rejected by both adapters. This establishes safe rejection, **not semantic quality**. The standalone helper can run; it does not establish signed-parent runtime acceptance. Apple Foundation Models reports Apple Intelligence disabled on this host. No settings were changed. A second isolated probe downloaded the already-supported `mlx-community/Qwen3-1.7B-4bit` model and used the production prompt, input fields and 30-second deadline. It returned JSON in 3.22 seconds and correctly joined the port-collision reports, but failed the other grouping/importance expectations; low-confidence and omitted reports remain separate through the adapter. This is partial capability evidence, not a passing semantic acceptance test. The model is cached locally; no application provider setting was changed.

## Release evidence

- Native iOS **0.1.13 (56)** archived, exported and uploaded successfully. Archive metadata was checked; App Store Connect reports `IN_BETA_TESTING` for the internal-only build (`3b04901b-c9d9-4e9b-af71-aa5d033eb777`). PR #139 was admin-merged as `1cc2f1cddf7bee98beb68cf51b620135bf374f4e`; its tree matches the source used for these artifacts.
- Desktop **0.1.20** built and signed; `codesign --verify --deep --strict` and bundle metadata passed. Installation remains pending.
- The signed desktop 0.1.20 parent/helper smoke check timed out after 120 seconds without output. Its sample is blocked in `_libsecinit_appsandbox` → `_xpc_pipe_routine` → `mach_msg2_trap`, before `main()`. The earlier new 0.1.18 and installed 0.1.17 checks stalled at the same boundary. Native computer-control still reports the Mac locked. Standalone helper inference works, but does not pass the signed-parent or installed-app runtime gate.
