...
The inspected artifacts are desktop **0.1.26** and native iOS **0.1.13 (63)**. [The permanent release record](releases/2026-09-24-source-preview-release.json), captured from `/tmp/skim-source63-release-evidence.json`, records all **21 shared C symbols** in the release evidence and **71 shared Swift policy symbols each** in the desktop helper and iOS evidence. The bundled helper's machine-code section matches the freshly compiled helper; whole-file hashes differ after signing. The recorded iOS executable/dSYM UUID is `E1D34C73-7372-3F9C-8DB8-1F932B2F7DE6`. These are release-path inclusion checks, not an assertion that the updated desktop is installed or that either native UI passed interactive acceptance. The signed desktop parent again timed out after 120 seconds before `main()` in `_libsecinit_appsandbox`; the locked Mac prevents native interactive acceptance. The release record confirms App Store Connect build 63 is `VALID` / `IN_BETA_TESTING`; TestFlight availability does not establish installed native runtime acceptance. [Current review and quality boundaries](CATCHUP_REVIEW.md#source-verified-newspaper-previews) distinguish browser/backend testing, model samples and native runtime evidence.
