...
The inspected artifacts are desktop **0.1.28** and native iOS **0.1.13 (65)**. [The current release record](releases/2026-09-24-summary-cancellation-release.json) records all **21 shared C symbols** and **71 shared Swift policy symbols each** in the desktop helper and iOS evidence. The bundled helper's machine-code section matches the freshly compiled helper; whole-file hashes differ after signing. The iOS executable/dSYM UUID is `BE674A5F-571F-3C7E-A578-347AB7EE675A`. These are release-path inclusion checks, not an assertion that the updated desktop is installed or that either native UI passed interactive acceptance. The signed desktop parent again timed out after 120 seconds before `main()` in `_libsecinit_appsandbox`; the last native-control check reported a locked Mac. Build 65 is `VALID` / `IN_BETA_TESTING` for internal testers.
