...
Initial audit: 2026-09-20; shared policy and source-preview update: 2026-09-24. Evidence includes source/build paths and inspection of the release artifacts described below. It does not establish installed-app runtime acceptance or complete feature parity. The feature matrix distinguishes resolved and remaining issues. Historical validation below refers to the initial audit; current checks and releases are recorded in `docs/CATCHUP_REVIEW.md`. Paths and line numbers may move with subsequent edits.

## Main finding

The functional desktop app and native iOS app still have separate domain engines, with bounded shared story and local-inference policy modules added on 2026-09-24. Desktop uses React, Tauri and Rust. Native iOS uses SwiftUI and the Swift `SkimCore` package, with substantial additional business logic in its application target. Both now compile and execute `shared/SkimStoryPolicy` for matching thresholds/classification, confidence, base ranking, single-source protection and stable identity hashing. They also share semantic edition instructions, group validation, pair-verification prompts, complete-clique partitioning, isolated event-rating instructions/validation and importance score adjustment. Today previews additionally share excerpt-selection/retry instructions, source bounds, exact-passage validation and cache evidence versioning; configured-provider calls and storage adapters remain separate. Native iOS and the desktop Swift helper additionally consume `shared/SkimInferencePolicy` for model families, end-of-turn tokens, thinking flags, sampling presets, output cleanup, chat-template availability checks and structured local chat-message handling. Similar models, SQLite tables and remaining algorithms are still parallel implementations.
...
| AI Inbox, summaries, Catch Up, Ask | `commands/ai.rs:334,1067,1520`; `commands/chat.rs:29,218` | `Views/ArticleListView.swift:171-172,255-285`; `Support/NativeAI.swift` | Partial: Today grouping/rating/preview policy is shared C, with shared local-inference policy. Other prompts, context selection, provider orchestration and caches remain separate. |
...
The inspected artifacts are desktop **0.1.27** and native iOS **0.1.13 (64)**. [The permanent release record](releases/2026-09-24-reader-ai-release.json) records all **21 shared C symbols** and **71 shared Swift policy symbols each** in the desktop helper and iOS evidence. The bundled helper's machine-code section matches the freshly compiled helper; whole-file hashes differ after signing. The iOS executable/dSYM UUID is `61C46F93-717E-3269-AE30-BB9CD28257FA`. These are release-path inclusion checks, not an assertion that the updated desktop is installed or that either native UI passed interactive acceptance. The signed desktop parent again timed out after 120 seconds before `main()` in `_libsecinit_appsandbox`; the locked Mac prevents native interactive acceptance. Build 64 was uploaded successfully; the release record captures Apple's processing and internal beta state.

The latest adapter fixes make summary caches depend on actual source evidence and preserve successful reader extraction for offline reuse. Native ordinary RSS summary/chat now use the reader cache and bounded resolver; provider/model overrides, preset word counts, generated-summary context and local web citations match the intended desktop behavior. `AIRequestPolicy` centralizes these native entry-point rules within `SkimCore`; the Rust desktop request policy is still separate. These fixes establish the tested parity boundaries, not additional shared implementation across shipping platforms. [Reader quality evidence](releases/2026-09-24-reader-ai-quality.json) records 431 passing tests and real desktop command/browser probes. Detailed-summary chronology remains defective on some real-model samples; no generic prompt change was adopted after a mixed 14-call experiment.
